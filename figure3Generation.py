...
ax[2].plot(yRefsToPlot, np.array(xRefsToPlot)+26.5, color = 'blue')
...
for i in indexes:
    ...
    ax[2].scatter(cords[:, 1], cords[:, 0]+26.5, color = 'green')
    ax[2].scatter(yRefsToPlot[i], xRefsToPlot[i]+26.5, color = 'blue')
...
ax[3].imshow(grid[0:midpoint+offset, 0:85].T.astype(int).T, cmap=cmap, norm=norm, origin="lower", interpolation=None)

ax[3].set_xlim(0,100)
ax[3].set_ylim(20,60)
ax[3].scatter(25, 36.5, color='red', s=100)
ax[3].plot(xCommand[0:tEnd-timeMulti-1]+25, yCommand[0:tEnd-timeMulti-1]-13.5, color = 'blue')
...
ax[3].set_xlabel('Downrange distance (m)')
